...
// Ajoutez le mutex nécessaire pour gérer la concurrence.
pthread_mutex_t mutex_manger;
...
void prendre_couverts(int id_philosophe){
  pthread_mutex_lock(&mutex_manger);
}

void poser_couverts(int id_philosophe){
  pthread_mutex_unlock(&mutex_manger);
}
...
int main(int argc, char *argv[]){
  ...
  pthread_mutex_init(&mutex_manger, NULL);
  ...
  pthread_mutex_destroy(&mutex_manger);
